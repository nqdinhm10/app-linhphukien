...
<?php
	...
	class product
	{
		...
		public function insert_product($data,$files){
			...
			if($productName=="" || $productQuantity=="" || $brand=="" || $category=="" || $product_desc=="" || $price=="" || $type=="" || $file_name ==""){
				...
				if($result){
					...
					$alert = "<span class='error'>Thêm thành công</span>";
					return $alert;
				}
			}
		}
		...
		public function show_countproduct(){
			$query = "SELECT COUNT(*) AS countproduct FROM tbl_product";
			$result = $this->db->select($query);
			return $result;
		}
		...
		public function insertWishlist($productid, $customer_id){
			...
			if($result_check_wlist){
				$msg = "<span class='error'>Sản phẩm đã được thêm vào yêu thích</span>";
				return $msg;
			}else{
